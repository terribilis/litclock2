...
class ImageGenerator:
    ...
    def create_image(self, time_str, quote_data):
        ...
        try:
            ...
            time_w, time_h = self.time_font.getbbox(display_time)[2:4]
        ...
        try:
            ...
            # For newer versions of Pillow
            _, _, _, quote_text_height = self.quote_font.getbbox(wrapped_quote)
        ...
        if self.config.get('show_book_info', True):
            ...
            # Handle different versions of PIL
            try:
                source_width = black_draw.textsize(source_text, font=self.source_font)[0]
            except AttributeError:
                # For newer versions of Pillow
                source_width = self.source_font.getbbox(source_text)[2]
                
            black_draw.text(
                (self.width - 60 - source_width, source_y),
                source_text,
...
def main():
    ...
    if black_path and red_path:
        logger.info(f"Generated: {black_path} and {red_path}")
    else:
